<?php
    ...
    <div class="x_panel">
        <div class="x_title">
            <h2><?php echo __('Openais'); ?></h2>
            <div class="clearfix"></div>
        </div>

        <div class="x_content"> 
            <div class="row">
                <?php echo $this->Form->create('Openai',array('url'=>array('action'=>'index'),'id'=>'filter-form','class'=>'form-inline','role'=>'form','novalidate'));?>
                <div class="col-md-12">
            ...
            <div class="table-responsive" id="datarows">
                ...
                <table class="table table-bordered table-hover has-checked-item">
                    ...
                    <tbody>
                        <?php $stt = (($this->Paginator->params['paging']['Openai']['page'] - 1) * $this->Paginator->params['paging']['Openai']['limit']) + 1; ?>
<?php foreach ($openais as $openai): ?>
<tr id="row-<?php echo $openai['Openai']['id'] ?>">
                        		<td><?php echo $stt++;?></td>

                        		<td class=""><?php echo h($openai['Openai']['prompt']); ?>&nbsp;</td>
		<td class=""><?php echo h($openai['Openai']['completion']); ?>&nbsp;</td>
		<td class="">
			<?php echo $this->Html->link($openai['User']['ho_va_ten_khoa_code'], array('controller' => 'users', 'action' => 'view', $openai['User']['id'])); ?>
		</td>
		<td class=""><?php echo h($openai['Openai']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link(__('<i class="glyphicon glyphicon-pencil"></i>'), array('action' => 'edit', $openai['Openai']['id']), array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'edit')); ?>
		</td>
                    <td>
                        <input type = "checkbox" class = "flat" name = "selete-item" value="<?php echo $openai['Openai']['id'] ?>">
                    </td>
                    	</tr>
<?php endforeach; ?>
                    </tbody>
...
<script>
    ...
    $('#filter-form').on('submit', function (e) {
        ...
        $.post("http://localhost/tlc_portal//admin/openais/index", data, function (response) {
    ...
    $(document).on("click", "#delete-seleted", function () {
        ...
        if (confirm("Thao tác này không thể phục hồi, bạn chắc chắn muốn thực hiện ?")) {
            var selectedRecord = $(".has-checked-item input[name='selete-item']:checked").serializeArray();
            $.post('http://localhost/tlc_portal//admin/openais/delete', {selectedRecord: selectedRecord}, function (response) {
                if (response) {
