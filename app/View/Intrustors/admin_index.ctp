<?php
    ...
    <div class="x_panel">
        <div class="x_title">
            <h2><?php echo __('Intrustors'); ?></h2>
            <div class="clearfix"></div>
        </div>

        <div class="x_content"> 
            <div class="row">
                <?php echo $this->Form->create('Intrustor',array('url'=>array('action'=>'index'),'id'=>'filter-form','class'=>'form-inline','role'=>'form','novalidate'));?>
                <div class="col-md-12">
...
<?php foreach ($intrustors as $intrustor): ?>
<tr id="row-<?php echo $intrustor['Intrustor']['id'] ?>">
                        		<td><?php echo $stt++;?></td>

                        		<td class="">
			<?php echo $this->Html->link($intrustor['Workshop']['name'], array('controller' => 'workshops', 'action' => 'view', $intrustor['Workshop']['id'])); ?>
		</td>
		<td class="">
			<?php echo $this->Html->link($intrustor['User']['name'], array('controller' => 'users', 'action' => 'view', $intrustor['User']['id'])); ?>
		</td>
...
<script>
    ...
    $('#filter-form').on('submit', function (e) {
        ...
        $.post("http://localhost/tlc_portal//admin/intrustors/index", data, function (response) {
    ...
    $(document).on("click", "#delete-seleted", function () {
        ...
        if (confirm("Thao tác này không thể phục hồi, bạn chắc chắn muốn thực hiện ?")) {
            var selectedRecord = $(".has-checked-item input[name='selete-item']:checked").serializeArray();
            $.post('http://localhost/tlc_portal//admin/intrustors/delete', {selectedRecord: selectedRecord}, function (response) {
                if (response) {
