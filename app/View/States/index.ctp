<?php
    ...
    <div class="x_panel">
        ...
        <div class="x_content"> 
            <div class="row">
                <?php echo $this->Form->create('State',array('url'=>array('action'=>'index'),'id'=>'filter-form','class'=>'form-inline','role'=>'form','novalidate'));?>
                <div class="col-md-12">
            ...
            <div class="table-responsive" id="datarows">
                ...
                <table class="table table-bordered table-hover has-checked-item">
                    ...
                    <tbody>
                        <?php $stt = (($this->Paginator->params['paging']['State']['page'] - 1) * $this->Paginator->params['paging']['State']['limit']) + 1; ?>
<?php foreach ($states as $state): ?>
<tr id="row-<?php echo $state['State']['id'] ?>">
                        		<td><?php echo $stt++;?></td>

                        		<td class=""><?php echo h($state['State']['name']); ?>&nbsp;</td>
		<td class=""><?php echo h($state['State']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link(__('<i class="glyphicon glyphicon-pencil"></i>'), array('action' => 'edit', $state['State']['id']), array('class' => 'btn btn-warning btn-xs', 'escape' => false, 'data-toggle'=>'tooltip', 'title' => 'edit')); ?>
                    ...
                    <td>
                        <input type = "checkbox" class = "flat" name = "selete-item" value="<?php echo $state['State']['id'] ?>">
                    </td>
                    	</tr>
<?php endforeach; ?>
                    </tbody>
...
<script>
    ...
    $('#filter-form').on('submit', function (e) {
        ...
        $.post("http://celri.tvu.edu.local/admin/states/index", data, function (response) {
    ...
    $(document).on("click", "#delete-seleted", function () {
        ...
        if (confirm("Thao tác này không thể phục hồi, bạn chắc chắn muốn thực hiện ?")) {
            var selectedRecord = $(".has-checked-item input[name='selete-item']:checked").serializeArray();
            $.post('http://celri.tvu.edu.local/admin/states/delete', {selectedRecord: selectedRecord}, function (response) {
                if (response) {
