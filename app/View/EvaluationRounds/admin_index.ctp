...
?><div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
        <div class="x_title">
            <h2>DANH SÁCH ĐỢT ĐÁNH GIÁ</h2>
            <div class="clearfix"></div>
        </div>

        <div class="x_content"> 
            <div class="row">
                <?php echo $this->Form->create('EvaluationRound', array('url' => array('action' => 'index'), 'id' => 'filter-form', 'class' => 'form-inline', 'role' => 'form', 'novalidate')); ?>
                <div class="col-md-12">

                    <?php echo $this->Form->input('title', array('placeholder' => 'Tên đợt', 'class' => 'form-control', 'div' => 'form-group', 'label' => array('class' => 'sr-only'))); ?>
                    <?php echo $this->Form->input('scholastic', array('placeholder' => 'Năm học', 'class' => 'form-control', 'div' => 'form-group', 'label' => array('class' => 'sr-only'))); ?>
                    <?php echo $this->Form->input('semester', array('placeholder' => 'Học kỳ', 'class' => 'form-control', 'div' => 'form-group', 'label' => array('class' => 'sr-only'))); ?>
...
<script>
    ...
    $('#filter-form').on('submit', function (e) {
        ...
        $.post("http://localhost/tlc_portal//admin/evaluationRounds/index", data, function (response) {
    ...
    $(document).on("click", "#delete-seleted", function () {
        ...
        if (confirm("Thao tác này không thể phục hồi, bạn chắc chắn muốn thực hiện ?")) {
            var selectedRecord = $(".has-checked-item input[name='selete-item']:checked").serializeArray();
            $.post('http://localhost/tlc_portal//admin/evaluationRounds/delete', {selectedRecord: selectedRecord}, function (response) {
                if (response) {
