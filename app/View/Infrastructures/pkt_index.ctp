            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group">
                        ...
                        <?php echo $this->Html->link('Bỏ lọc', array('action' => 'pkt_infrastructure_index'), array('class' => 'btn btn-warning btn-xs')); ?>
                    </div>
                </div>
...
<script>
    ...
    $('#filter-form').on('submit', function (e) {
        ...
        $.post("<?php echo BASE_URL ?>/pkt/infrastructures/pkt_index", data, function (response) {
    ...
    $(document).on("click", "#delete-seleted", function () {
        ...
        if (confirm("Thao tác này không thể phục hồi, bạn chắc chắn muốn thực hiện ?")) {
            var selectedRecord = $(".has-checked-item input[name='selete-item']:checked").serializeArray();
            $.post('http://celri.tvu.edu.local/admin/infrastructures/delete', {selectedRecord: selectedRecord}, function (response) {
                if (response) {
