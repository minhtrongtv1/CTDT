...
class NotifiableBehavior extends ModelBehavior {
     ...
     * 
     */
    public function getUnreadNotification(Model $Model, $user_id = null, $conditions = array(),$limit=false,$order=array()) {
        if (empty($user_id))
            $user_id = $Model->id;
        return $Model->Notification->getUnread($user_id, $limit, $conditions,$order);
    }
     ...
     */
    public function getLastNotification(Model $Model, $user_id, $limit = 1) {
