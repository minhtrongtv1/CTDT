...
<div class="row">
    ...
    <div class="col-md-2">
        <?php
        echo $this->Gallery->link(null, null, array('class' => 'btn btn-primary pull-right', 'style' => 'margin-top: 10px')
        );
        ?>
    </div>
</div>
...
<div class="row">
    <div class="col-md-12">
        <?php if ($search_status == "draft") { ?>
            <?php
            echo $this->Html->link(
                    '<i class="fa fa-check"></i> ' . __d('gallery', 'Các album đã công khai'), '?status=published', array('class' => 'btn btn-default', 'escape' => false)
            )
            ?>
        <?php } else { ?>
            <?php
            echo $this->Html->link(
                    '<i class="fa fa-pagelines"></i> ' . __d('gallery', 'Album nháp'), '?status=draft', array('class' => 'btn btn-default', 'escape' => false)
            )
            ?>
        <?php } ?>
    </div>
