...
class HabtmCounterCacheBehavior extends ModelBehavior {
    ...
    protected function _setNewHabtmIds($model) {
        ...
        foreach ($this->settings[$model->alias] as $habtmAlias => $settings) {
            ...
            $this->_habtmIds[$model->alias][$model->id][$habtmAlias] = array_unique(
                    array_merge($this->_habtmIds[$model->alias][$model->id][$habtmAlias], Hash::extract($model->data[$habtmAlias], $habtmAlias))
            );
        }
    }
    ...
    function _updateCounterCache(&$model) {
        ...
        foreach ($this->settings[$model->alias] as $habtmAlias => $settings) {
            ...
            $updateQuery = array(
                'table' => $ds->config['prefix'] . $model->$habtmAlias->table,
                'alias' => '`' . $habtmAlias . '`',
                'order' => null,
                'limit' => null,
                'group' => null,
            ...
            if ($settings['counterCache']) {
                ...
                $counterQuery = array(
                    'fields' => array('COUNT(*)'),
                    'table' => $ds->config['prefix'] . $settings['joinTable'],
                ...
                if ($settings['counterScope']) {
                    ...
                    $counterQuery['joins'][] = array(
                        'type' => 'INNER',
                        'alias' => $model->alias,
                        'table' => $ds->config['prefix'] . $model->table,
