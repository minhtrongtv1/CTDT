...
class AclController extends AclManagerAppController {
    ...
    public function drop() {
        ...
        $this->Flash->success(__("Đã xóa ACOs và AROs"));
    ...
    public function drop_perms() {
        if ($this->Acl->Aro->Permission->deleteAll(array("1 = 1"))) {
            $this->Flash->success(__("Permissions dropped"));
        } else {
    ...
    public function permissions() {
         ...
         */
        $rootControllerId = 4265;

        $orConditions = array(array('id' => $rootControllerId));
        if (!empty($this->request->query['id'])) {
            $requestId = $this->request->query['id'];
            $orConditions[] = array('parent_id' => $requestId);
            $orConditions[] = array('id' => $requestId);
        }

        $this->acos = $acos = $this->Acl->Aco->find('all', array(
            'order' => 'Aco.lft ASC',
            'recursive' => 1,
        ...
        //Lấy danh sách controller để fill vào trong filter form
        $controllersAcos = $this->Acl->Aco->find('first', array('conditions' => array('alias' => 'controllers')));
        $rootAcos = $this->Acl->Aco->find('list', array('fields' => array('id', 'alias'), 'order' => array('alias' => 'ASC'), 'conditions' => array('OR' => array(array('parent_id is NULL'), array('parent_id' => $controllersAcos['Aco']['id'])))));
        $this->request->data = array('Perms' => $perms);
        $this->set('aroAlias', $Aro->alias);
        $this->set('aroDisplayField', $Aro->displayField);
        $this->set(compact('acos', 'aros', 'rootAcos'));
    }
    ...
    public function update_acos() {
        ...
        if (count($knownAcos) > 0) {
            //$acoIds = Hash::extract('/Aco/id', $knownAcos);
            $acoIds = Hash::extract($knownAcos, '/Aco/id');
            $this->Acl->Aco->deleteAll(array('Aco.id' => $acoIds));
        }

        $this->Flash->success(sprintf(__("%d ACOs have been created/updated"), $count));
    ...
    public function update_aros() {
        ...
        $this->Flash->success(sprintf(__("%d AROs have been created"), $count));
        $this->redirect($this->request->referer());
    }
