 ...
 *
 * todo: 2015-02-26 Removed Localization Messages
 */
		...
		$this->_validationRules = [
			'formField' => [
				'between' => [
					...
					'message' => __d('tools', 'Enter %s to %s Characters', PWD_MIN_LENGTH, PWD_MAX_LENGTH),
					'allowEmpty' => null,
					'last' => true,
				]
			],
			'formFieldRepeat' => [
				'validateNotEmpty' => [
					...
					'message' => __d('tools', 'Enter Your New Password Again'),
					'allowEmpty' => true,
					'last' => true,
				],
				'validateIdentical' => [
					...
					'message' => __d('tools', 'New Passwords Do Not Match'),
					'allowEmpty' => null,
					'last' => true,
				],
			],
			'formFieldCurrent' => [
				'notEmpty' => [
					...
					'message' => __d('tools', 'Enter Current Password'),
					'allowEmpty' => null,
					'last' => true,
				],
				'validateCurrentPwd' => [
					'rule' => 'validateCurrentPwd',
					//'message' => __d('tools', 'valErrCurrentPwdIncorrect'), //not using internationalization
					'message' => __d('tools', 'Invalid Current Password'),
					'allowEmpty' => null,
					'last' => true,
				]
			]
		...
		foreach ($rules as $field => $fieldRules) {
			foreach ($fieldRules as $key => $rule) {
				...
				if ($key === 'between') {
					...
					$rule['message'] = __d('tools', 'Enter %s to %s Characters', $this->settings[$Model->alias]['minLength'], $this->settings[$Model->alias]['maxLength']);
				}
		...
		if ($this->settings[$Model->alias]['current'] && !isset($Model->validate[$formFieldCurrent])) {
			...
			if (!$this->settings[$Model->alias]['allowSame']) {
				$Model->validator()->add($formField, 'validateNotSame', [
					...
					'message' => __d('tools', 'Password is same as before.'),
