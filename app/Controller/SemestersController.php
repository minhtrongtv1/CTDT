...
//$conditions = Set::merge($conditions, array('Semester.fieldName' => $value));
    ...
    public function edit($id = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            if ($this->Semester->save($this->request->data)) {
                $this->Flash->success(__('semester đã được lưu'));
                $this->redirect(array('action' => 'index'));
            } else {
                $this->Flash->error(__('semester lưu không thành công, vui lòng thử lại.'));
            }
        } else {
            $options = array('conditions' => array('Semester.' . $this->Semester->primaryKey => $id));
    ...
    public function delete($id = null) {
        ...
        if ($this->Semester->delete()) {
            $this->Flash->success(__('Semester đã xóa'));
            $this->redirect(array('action' => 'index'));
        } else {
            $this->Flash->error(__('Semester xóa không thành công'));
            $this->redirect(array('action' => 'index'));
        }
    }
}
